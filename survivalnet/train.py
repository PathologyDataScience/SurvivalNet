...
def train(pretrain_set,
	...
	"""
    finetune_lr = theano.shared(
        numpy.asarray(finetune_config['ft_lr'], dtype=theano.config.floatX))
    ...
    model = Model(numpy_rng=numpy_rng,
    ...
    if pretrain_config is not None:
        n_batches = len(train_set) / (pretrain_config['pt_batchsize']
                                      or len(train_set))

        pretraining_fns = model.pretraining_functions(
            pretrain_set, pretrain_config['pt_batchsize'])
        ...
        if verbose:
            print(('Pretraining took {} minutes.'.format(
    ...
    for epoch in range(finetune_config['ft_epochs']):
        ...
        if optim == 'BFGS':
            ...
            train_cost, train_risk, train_features = test(
                train_set['X'], train_set['O'], train_set['A'], 1,
                *train_masks)
        elif optim == 'GDLS':
        ...
        # training cost, risk, and cindex at the same time.
            ...
            train_cost, train_risk, train_features = train(
                train_set['X'], train_set['O'], train_set['A'], 1,
                *train_masks)
        train_ci = survivalAnalysis.c_index(train_risk, train_set['T'],
                                            1 - train_set['O'])

        # Calculates testing cost, risk and cindex using th eupdated model.
        test_cost, test_risk, _ = test(test_set['X'], test_set['O'],
                                       test_set['A'], 0, *test_masks)
        test_ci = survivalAnalysis.c_index(test_risk, test_set['T'],
                                           1 - test_set['O'])
        ...
        if earlystp and epoch >= 15 and (epoch % 5 == 0):
            ...
            if check:
                print((('Training Stopped Due to Overfitting! cindex = {},'
                        ' MaxIter = {}').format(test_cindices[max_iter],
                                                max_iter)))
                break
        else:
            max_iter = epoch
    ...
    if verbose:
        print('C-index score after {} epochs is: {}'.format(
            max_iter, max(test_cindices)))
