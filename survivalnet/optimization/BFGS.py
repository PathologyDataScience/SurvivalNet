...
class BFGS(object):
    def __init__(self, model, x, o, atrisk):
        ...
        self.theta_shape = sum(
            [self.params[i].get_value().size for i in range(len(self.params))])
        self.old_old_fval = None
        N = self.theta_shape
        self.H_t = numpy.eye(N, dtype=numpy.float32)

        self.theta = theano.shared(
            value=numpy.zeros(self.theta_shape, dtype=theano.config.floatX))
        self.theta.set_value(numpy.concatenate(
            [e.get_value().ravel() for e in self.params]),
                             borrow="true")

        self.gradient = theano.function(on_unused_input='ignore',
                                        inputs=[is_tr] + model.masks,
                                        outputs=T.grad(self.cost(o, atrisk),
                                                       self.params),
                                        givens={
                                            model.x: x,
                                            model.o: o,
                                            model.at_risk: atrisk,
                                            model.is_train: is_tr
                                        },
                                        name='gradient')
    ...
    def f(self, theta_val):
        ...
        for i in range(len(self.params)):
            p = self.theta.get_value()[idx:idx +
                                       self.params[i].get_value().size]
    ...
    def bfgs_min(self, f, x0, fprime):
        ...
        self.H_t += denom * denom * numpy.dot(
            self.phi_t,
            Z) * delta_t[:, numpy.newaxis] * delta_t[numpy.newaxis, :]
        return theta_next
