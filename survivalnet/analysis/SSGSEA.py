...
def SSGSEA(Gradients, Symbols, Sets, Alpha=0.0, Normalize=False):
    ...
    ES = np.zeros((Gradients.shape[0], len(Sets))) * np.nan

    # rank expression values within each sample
    Ranks = np.zeros((Gradients.shape[0], len(Symbols)))
    if Normalize:  # rank z-scored gradient values (normalize each feature)
        Gradients = sp.stats.mstats.zscore(Gradients, axis=0)
    for j in np.arange(Gradients.shape[0]):
        Ranks[j, :] = sp.stats.rankdata(-Gradients[j, :], method='ordinal')
