...
def Run () :
        ...
        for shuffle in xrange(num_shuffles):
            ...
            sa = SurvivalAnalysis()    
            train_set['X'], train_set['T'], train_set['C'], train_set['A'] = sa.calc_at_risk(X[0:fold * 8,], T[0:fold * 8], C[0:fold * 8]);
            test_set['X'], test_set['T'], test_set['C'], test_set['A'] = sa.calc_at_risk(X[fold * 8: ,], T[fold * 8:], C[fold * 8:]);
